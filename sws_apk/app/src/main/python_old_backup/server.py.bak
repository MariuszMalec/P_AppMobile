...
def prepare_database(log_path):
    ...
    meetings.db znajdujący się obok kodu Python jest
    bazą startową wbudowaną w APK.
    """

    log_dir = Path(log_path).resolve().parent
    writable_db = log_dir / "meetings.db"

    bundled_db = (
        Path(__file__).resolve().parent / "meetings.db"
    )
