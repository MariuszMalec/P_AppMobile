...
def prepare_database(log_path):
    ...
    bundled_db = os.path.join(base_dir, "sws.db")

    writable_dir = os.path.dirname(log_path)
    writable_db = os.path.join(writable_dir, "sws.db")
    ...
    if not os.path.exists(bundled_db):
        log_message(log_path, "BLAD: brak bundled sws.db")
        raise FileNotFoundError(bundled_db)

    if not os.path.exists(writable_db):
        shutil.copy2(bundled_db, writable_db)
        log_message(log_path, "Skopiowano sws.db do katalogu zapisywalnego")
    else:
        log_message(log_path, "Writable sws.db juz istnieje")
